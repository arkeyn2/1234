...
public interface IInscripcionDao extends CrudRepository<Inscripcion, Long>{
	...
	*/
	...
	@Query("select ins from Inscripcion ins inner join Usuario usu on usu.id=ins.usuario.id where ins.mascota.id=?1")
	public List<Inscripcion> findReservaPorMascota(Long idMascota);
